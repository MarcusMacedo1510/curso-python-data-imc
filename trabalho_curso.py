...
Original file is located at
    https://colab.research.google.com/github/MarcusMacedo1510/curso-python-data-imc/blob/main/trabalho_curso.ipynb
...
class Iris():
    ...
    def TratamentoDeDados(self):
        ...
        for col in self.df.columns.tolist():
          print('Número de valores ausentes na coluna {}: {}'.format(col, self.df[col].isnull().sum()))
    ...
    def Treinamento_LinearRegression(self):
        ...
        Nota: Esta função deve ser ajustada conforme o modelo escolhido.
        """
        print('\n\n TREINAMENTO COM MÉTODO DE REGRESSÃO LINEAR\n')
        # Fazendo uma cópia do Dataset original

        iris_data = self.df.copy()

        print('Selecionando conjunto de dados numéricos (PetalLengthCm, PetalWidthCm)  para a Flor: Iris-virginica\n')
        # Selecionando uma Espécie de Flor = Iris-virginica
        data = iris_data[iris_data['Species'] == 'Iris-virginica'].copy()
        data.drop(columns=['Species','SepalLengthCm','SepalWidthCm'],inplace=True)
        print('Dados selecionados:')
        print(f'data: \n{data.head(10)}')

        print('\Plotando DataFrame (PetalLengthCm, PetalWidthCm)  para a Flor: Iris-virginica\n')
        ...
        plt.title('Gráfico Scatter de Petal (Iris-virginica): Length x Width')
        plt.show()
        ...
        # Plotando Dataset de Treinamento
        print('\nPlotando DataFrame de Treinameno (PetalLengthCm, PetalWidthCm)  para a Flor: Iris-virginica\n')
    ...
    def Teste_LinearRegression(self):
        ...
        plt.plot(self.X_teste, self.model.predict(self.X_teste) )
        plt.show()
